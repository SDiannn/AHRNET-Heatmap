...
class PoolAttnFormer(nn.Module):
    """
    PoolAttnFormer, the main class of our model
    ...
    def forward(self, x):
        ...
        if self.fork_feat:
            ...
            return x
        x1 = self.norm(x)
        cls_out = self.head(x1.mean([-2, -1]))
        # for image classification
        return x1, cls_out


class PoolAttnFormer_xin(nn.Module):
    """
    PoolAttnFormer, the main class of our model
    --layers: [x,x,x,x], number of blocks for the 4 stages
    --embed_dims, --mlp_ratios, --pool_size: the embedding dims, mlp ratios and
    ...
    def forward(self, x):
        ...
        x, out = self.forward_tokens(x)    #x= 1,512,7,7
        if self.fork_feat:
            x1 = self.norm(x)
            # otuput features of four stages for dense prediction
            return x1, out
...
def poolattnformer_s24(pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = torch.load("./pool24/model_best.pth.tar")
        model.load_state_dict(checkpoint["state_dict"], False)
    return model



def poolattnformer_s12_xin(pretrained=False, **kwargs):
    """
    poolattnformer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
...
def poolattnformer_s24_xin(pretrained=False, **kwargs):
    """
    poolattnformer-S12 model, Params: 12M
    --layers: [x,x,x,x], numbers of layers for the four stages
    --embed_dims, --mlp_ratios:
