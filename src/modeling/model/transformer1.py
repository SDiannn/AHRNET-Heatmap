...
class Transformer_4de(nn.Module):
    ...
    def forward(self, img_features, cam_with_jv, pos_embed, attention_mask1=None, attention_mask2=None):
        device = img_features.device
        hw, bs, _ = img_features.shape # (height * width), batch_size, feature_dim(49,1,512)
        mask = torch.zeros((bs, hw), dtype=torch.bool, device=device) # batch_size X (height * width)


        zero_tgt = torch.zeros_like(cam_with_jv)

        cam_with_jv_features1 = self.decoder(cam_with_jv, img_features, tgt_mask=attention_mask1,    #217,1,512
                                   memory_key_padding_mask=mask, pos=pos_embed, query_pos=zero_tgt) # (num_joints + num_vertices) X batch_size X feature_dim

        e_outputs = self.encoder(img_features, src_key_padding_mask=mask, pos=pos_embed)

        cam_with_jv_features = self.decoder(cam_with_jv_features1, e_outputs, tgt_mask=attention_mask2,  # 217,1,512
                                            memory_key_padding_mask=mask, pos=pos_embed,
                                            query_pos=zero_tgt)

        cam_features, jv_features = cam_with_jv_features.split([1, 216], dim=0)

        return cam_features, img_features, jv_features
...
def build_transformer_4de(transformer_config):
    return Transformer_4de(model_dim=transformer_config['model_dim'],
